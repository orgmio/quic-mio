...
func (c *Config) Clone() *Config {
	copy := *c
	copy.AdditionalClientTransportParameters = make([]TransportParameter, len(c.AdditionalClientTransportParameters))
	for i, parameter := range c.AdditionalClientTransportParameters {
		copy.AdditionalClientTransportParameters[i] = TransportParameter{
			ID:    parameter.ID,
			Value: append([]byte(nil), parameter.Value...),
		}
	}
	return &copy
}
