...
func NewCryptoSetupClient(
	...
	transportParameters := cs.ourParams.Marshal(protocol.PerspectiveClient)
	if tp.UseChromeClientHello {
		cs.conn = newChromeQUICClient(tlsConf, transportParameters)
	} else {
		cs.conn = newStandardQUICClient(tlsConf)
		cs.conn.SetTransportParameters(transportParameters)
	}
...
func NewCryptoSetupServer(
	...
	cs.tlsConf = tlsConf
	cs.conn = wrapStandardQUICConn(tls.QUICServer(getQUICConfig(tlsConf, localAddr, remoteAddr)))
	return cs
}
